...
function dbwebb-inspect()
{
    ...
    local command1=
    local command2="$SSH_CMD 'dbwebb-inspect --course-repo \"$DBW_REMOTE_BASEDIR/$DBW_COURSE\" $kmom $who' | tee '$log';"
...
dbwebb-selfupdate()
{
    ...
    local cmd="printf 'Downloading...'; wget $silent https://raw.githubusercontent.com/mosbth/dbwebb-cli/master/dbwebb2 -O /tmp/$$; printf ' installing...'; install /tmp/$$ /usr/local/bin/dbwebb; printf ' cleaning up...'; rm /tmp/$$; printf ' done.\n'"
    local message="to update dbwebb installation."
    executeCommand "$intro" "$cmd" "$message"
        
    dbwebb updateconfig
    
    printf "The updated version is: "
    dbwebb --version
}
